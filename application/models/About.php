...
//    public function add_to_cart($id,$ip_address,$quantity,$price)
//    {
//        $conn =App::$app->get_db();
//        $stmt=$conn->prepare("INSERT INTO cart(product_id,ip_address,quantity,price) VALUES(:id,:ip_address,:quantity,:price)");
...
//    {
//        $conn = App::$app->get_db();
//        $stmt = $conn->prepare("SELECT p.id as ProductId,c.product_id,c.ip_address,p.name,c.quantity,p.photo,p.price as productPrice,c.price as cartPrice,d.id,d.value_discount,d.product_id,d.end_date,p.discount FROM cart c LEFT JOIN products p ON p.id=c.product_id
